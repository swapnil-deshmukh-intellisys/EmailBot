...
export async function POST(req, { params }) {
  ...
  if (!result.ok) {
    ...
    await campaign.save();
    return NextResponse.json({ success: true, ok: true, message: result.message || 'Campaign paused.' }, { headers: NO_STORE_HEADERS });
  }
  ...
  return NextResponse.json({ success: true, ok: true, message: 'Campaign paused.' }, { headers: NO_STORE_HEADERS });
}
