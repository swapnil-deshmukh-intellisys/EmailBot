...
export async function POST(req) {
  ...
  const filter = { userEmail, status: { $in: ['pending', 'running'] } };
  if (projectId) filter.projectId = projectId;
  if (body.conversationId) filter._id = body.conversationId;
  const result = await WarmupConversation.updateMany(filter, { $set: { status: 'paused', updatedAt: new Date() } });
  return NextResponse.json({ success: true, paused: result.modifiedCount }, { headers: NO_STORE_HEADERS });
}
